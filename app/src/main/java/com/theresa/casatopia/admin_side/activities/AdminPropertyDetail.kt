...
class AdminPropertyDetail : AppCompatActivity(), OnMapReadyCallback {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        val ownershipProfile = findViewById<TextView>(R.id.ownershipProfile)
        val ownershipProfileCard = findViewById<CardView>(R.id.ownershipProfileCard)
        ...
        userDatabase.getSingleEstate(estateId){ estateFromRepository ->
            ...
            if(estateFromRepository.availability == false){
                acquire.isEnabled = false
                acquire.text = "Rented"

                duration.isVisible = true
                duration.text = "Duration of rent: ${estateFromRepository.purchase!!.duration} years"

                adminDatabase.getClientProfile(estateFromRepository.purchase!!.user) {
                    type.text = "Rented by: ${it.userFirstName} ${it.userLastName}"
                    ownershipProfile.text = "View ${it.userFirstName} ${it.userLastName} profile"

                    val userId = it.userId
                    ownershipProfile.setOnClickListener {
                        startActivity(
                            Intent(
                                context,
                                ClientProfile::class.java
                            ).putExtra("clientId", userId)
                        )
                    }
                ...
                ownershipProfileCard.isVisible = true
